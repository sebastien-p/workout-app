...
export class NativeService {
  ...
  private async beep(): Promise<void> {
    if (!this.beepBuffer) {
      this.beepBuffer = new Promise(async (resolve, reject) => {
        const response: Response = await fetch(beep);
    ...
    const node: AudioBufferSourceNode = this.audioContext.createBufferSource();
    node.connect(this.audioContext.destination);
    node.onended = () => node.disconnect();
    node.buffer = await this.beepBuffer;
    node.start();
  }
}
